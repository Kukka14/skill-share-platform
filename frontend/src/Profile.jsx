...
export default function Profile() {
  ...
  const fetchPostComments = async (postId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/interactions/posts/${postId}/comments`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/interactions/posts/${commentingPostId}/comments`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        console.error('Comment submission error:', response.status, responseData);
        throw new Error(`Failed to add comment: ${response.status} ${JSON.stringify(responseData)}`);
      }
  ...
  const handleSubmitEditComment = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/interactions/comments/${editingCommentId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!response.ok) {
        console.error('Comment update error:', response.status, responseData);
        throw new Error(`Failed to update comment: ${response.status} ${JSON.stringify(responseData)}`);
      }
      
      const updatedComment = responseData;
              ...
              {posts.map((post) => (
                <div key={post.id} className="border rounded-lg overflow-hidden bg-white">
                  ...
                  <div className="p-4 border-t">
                    ...
                    {/* Display comments count if available */}
                    ...
                    {/* Display first comment if available */}
                    {postComments[post.id] && postComments[post.id].length > 0 && (
                        ...
                        <div className="bg-gray-100 rounded-xl px-4 py-3 flex-1">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="flex items-center">
                              ...
                              {/* Show edit/delete buttons if this is the user's comment */}
